...
const fetchMeetingsByUserId = function (user_id, meeting_status) {
  ...
  return (`
    SELECT start_time, name, (select users.username FROM users WHERE users.id = meetings.owner_id) AS owner_username, meetings.id, status, users_meetings.link_to_notes, array_agg(users.username) AS invited_users FROM meetings
